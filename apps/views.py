...
from celery.states import FAILURE, PENDING, STARTED, SUCCESS, state
...
def wait_for_command(request: HttpRequest, app_name, task_id, after):
    ...
    if res.state == state(SUCCESS):
        return redirect_reverse(
            after, kwargs={"app_name": app_name, "task_id": task_id}
        )
...
def index(request: HttpRequest):
    try:
        ...
        if e.__class__.__name__ in [
            ...
            return render(
                request, "setup_key.html", {"key": tasks.get_public_key.delay().get()}
            )
        else:
            raise
...
def global_config_set(request):
    ...
    if form.is_valid():
        return run_cmd_with_log(
            None,
            "Setting %s" % form.cleaned_data["key"],
            "config:set --global %s=%s"
...
def remove_domain(request: HttpRequest, app_name):
    ...
    if letsencrypt(app_name) is not None:
        commands.append("letsencrypt %s" % app_name)
    return run_cmd_with_log(
        app_name, "Remove domain %s" % name, commands, "check_domain"
    )
...
def deploy(request: HttpRequest, app_name):
    if request.POST["action"] == "deploy":
        config = app_config(app_name)
        res = tasks.deploy.delay(
            app_name, request.POST["url"], config.get("GIT_BRANCH", "master")
        )
        clear_cache("config:show %s" % app_name)
        clear_cache("domains:report %s" % app_name)
        ...
        return redirect_reverse(
            "wait_for_command",
            kwargs={"app_name": app_name, "task_id": res.id, "after": "check_deploy"},
        )
    elif request.POST["action"] == "rebuild":
        return run_cmd_with_log(
            app_name, "Rebuilding", "ps:rebuild %s" % app_name, "check_rebuild"
        )
    else:
...
def create_redis(request: HttpRequest, app_name):
    return run_cmd_with_log(
        app_name,
        "Add Redis",
        ["redis:create %s" % app_name, "redis:link %s %s" % (app_name, app_name)],
        "check_redis",
    )
...
def create_app(app_name: str):
    ...
    return run_cmd_with_log(
        app_name, "Add app %s" % app_name, "apps:create %s" % app_name, "check_app"
    )
...
def github_webhook(request: HttpRequest):
    ...
    if data["ref"] != default_ref:
        return HttpResponse(
            "Push to non-default branch (saw %s, expected %s)"
    ...
    config = app_config(app.name)
    res = tasks.deploy.delay(app.name, clone_url, config.get("GIT_BRANCH", "master"))
    clear_cache("config:show %s" % app.name)
    return HttpResponse(
        "Running deploy. Deploy log is at %s"
        % request.build_absolute_uri(reverse("show_log", kwargs={"task_id": res.id}))
    )
